...
const BY_ID = new Map<string, RegistryReasoning>()
for (const [key, ids] of Object.entries(SETS)) {
  ...
  const defaultEffort = efforts.includes('high') ? 'high' : efforts[0] as string
  for (const id of ids) BY_ID.set(id, { efforts, defaultEffort })
}
 ...
 */
export function commandCodeReasoning(upstreamId: string): RegistryReasoning | undefined {
