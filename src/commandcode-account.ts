...
export class CommandCodeAccountService {
  private readonly cache = new Map<string, { at: number; value: CommandCodeAccountView }>()
  private readonly failedAt = new Map<string, number>()

  constructor(private readonly hooks: CommandCodeAccountHooks) {}
   ...
   */
  async read(group: ResolvedGroup, signal?: AbortSignal): Promise<CommandCodeAccountView> {
    ...
    let credits: { body?: unknown; status?: number; error?: string }
    let usage: { body?: unknown; status?: number; error?: string }
    try {
    ...
    for (const half of [credits, usage]) {
      if (half.error !== undefined) this.hooks.log(`${COMMANDCODE.ns}: ${half.error}`)
    }
    return view
  }
   ...
   */
  async readCached(group: ResolvedGroup, signal?: AbortSignal): Promise<CommandCodeAccountView> {
    const key = group.key
...
function buildView(
  credits: { body?: unknown; status?: number; error?: string },
  usage: { body?: unknown; status?: number; error?: string },
): CommandCodeAccountView {
 ...
 */
export function commandCodeAccountFetchHandler(
  service: CommandCodeAccountService,
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      const view = await service.readCached(selected[0] as ResolvedGroup, request.signal)
