...
## [1.1.0] — 上下文选择修复、Command Code 思考深度、菜单栏重设计

三件事，全部由真实渲染/真实数据验证，而非推断。

### 1. 上下文选择：点不动 + 整页闪烁

**两个独立的缺陷，用户看到的是同一个现象。**

**点不动**：档位按钮和「已选中集合」来自**两个不同的推导**。当二者不一致时（很常见），一行只会显示**一个**按下的档位——而它恰好是「至少保留一个」守卫**禁用的那个**。于是整行看起来只有一个可用控件，且点它毫无反应。

根因更深一层：档位**集合**和**按下状态**被互相推导，导致关掉一档后那个按钮就消失了——刚被使用的控件在指针下消失。现在两者分离：**可选项恒定**（关掉的档位仍然可见、可重新勾选），只有按下状态跟随存储值。

**整页闪烁**：每次写入后都重新拉取**整份设置**和**全部四个分组**的凭据状态，然后替换页面 state——一次往返 + 全子树重渲染，期间**所有**控件被禁用。

实际上 `mutate` 的返回值**已经携带提交后的配置**，所以现在就地采用它。凭据状态不可能因设置写入而改变，重新拉取纯属浪费。只有**被拒绝或冲突**时才回退到完整重载（那才是真正需要它的场景）。

### 2. Command Code：终于能选思考深度了

**为什么之前不能选**：Command Code 的模型列表只carrying路由信息（7 个字段，无任何 reasoning 字段），而它的能力页只回答「能否推理」这个**布尔值**，从不给出深度列表。两个来源对「effort」都是沉默的，于是所有模型都落到分组默认值——而 Command Code **刻意没有**分组默认值（因为它的档位是逐模型的）。结果：没有任何模型提供思考控件。

**修复**：从厂商自己的 CLI 目录取得逐模型档位，并加了一个 family 级钩子。
...
档位**逐模型且不统一**（7 种不同组合），所以单一分组词表会送出模型拒绝的档位。目录中**没有**档位的模型（29 个）不提供控件，而不是伪造一个。

**顺带修掉一个会 400 的 bug**：Command Code 默认用 `toggle` 拼法，会额外发送 `thinking: {type}` 块——而该网关**只在 Anthropic wire 上接受它**。厂商 BYOK 文档明确写 OpenAI 面只取 `reasoning_effort`，其 CLI 也只发这一个字段。已改为 `effort-only`。

**还有一个静默的连接遗漏**：`claude-haiku-4-5-20251001` 在列表里带日期、能力页里不带，于是它**同时丢失**了 reasoning 和 vision 标记。已修复。

### 3. 菜单栏重设计

原来**每一行**都有四个控件簇（可见性、上下文档位、视觉、星标）。一个分组 60–70 行 ≈ **约 500 个控件**。

依据检索到的设计系统证据（Cloudscape 的「in-context actions 只保留一两个，其余收进下拉」、NN/g 的 progressive disclosure、WCAG 2.2 SC 2.5.8 的 24×24 目标尺寸），改为：

- **折叠态**：一行摘要——`step-3.7-flash │ 200K · 256K · 400K · 1M │ vision │ ▸`——只留**一个**控件
- **展开态**：带标签的分组字段（上下文 / 视觉 / 置顶 / 能力）
- 摘要**本身就说明状态**：渐进披露的前提是折叠态仍能回答问题，而不是把问题藏起来
- **多行可同时展开**：这个面板的任务是**比较模型**，手风琴式的单开会把每次比较变成两步操作
- 控件尺寸满足 WCAG 2.2 的 24×24 下限（此前四连小按钮不满足）

### 4. 安全加固（独立审计）

一轮独立安全审计发现了下列问题，全部已修复并加了回归测试：

| 严重度 | 问题 | 影响 |
| --- | --- | --- |
| **严重** | 调试用的 `console.warn` 进了**三个发布产物** | 每次设置写入都把整份配置打到浏览器控制台（凭证**引用**列表、模型可见性等；密钥值不在其中） |
| 高 | 能力页扫描是**平方级**的 | 一个 512KiB 的恶意页面可冻结 Host 事件循环 **37 秒**（8MiB 上限下约 16 分钟）；现已单趟线性，同样输入 < 2 秒 |
| 高 | Command Code 账户路由**接受所有 HTTP 方法** | POST 洪水可每 5 秒消耗一次真实上游请求（带密钥），无限持续 |
| 中 | 上游错误文本进入面板 | 恶意上游可伪造看起来像插件自己写的提示 |
| 中 | 能力页与订阅读取**无字节上限**，且跟随重定向 | 内存无上限；携带凭证的请求会被重定向带走 |
| 低 | Fusion `applyLeader` 在无会话时**静默成功** | 界面报告成功但什么都没做 |
| 低 | 密钥回显进错误消息 | 把字面密钥粘进引用字段时，整串会进错误、日志与截图 |

### 5. 模型元数据修正（独立审计）

**最重要的一处**：模型可以被提供**超过自身窗口**的上下文档位。Command Code 的列表在**每一行**都公布 `context_length`，但有 **25/82 行**被提供了超出窗口的档位——最差的是 200K 模型被提供 **1M（5 倍虚报）**，而且上报给 harness 的窗口跟着那个档位走。

根因是一句注释的假设被反转了：过滤被跳过，理由是「未收录模型只有下限猜测」——但该端点**公布了**这个数字。现在区分「端点公布的窗口」和「我们猜的下限」。

其余修正：

- **两个族的禁用词相反**（实测）：relay 用 `none`、拒绝 `off`；Go 用 `off`、拒绝 `none`。四个 relay 条目写错了拼法——只是碰巧没被触发。
- **补录 5 个 relay 模型**（全部七档 effort 与图片输入实测 200），并拒用 1 个实测 400 的。
- **两个推理词表比实际能力窄**：`kimi-k3` 实测接受 minimal/medium/xhigh/max，M3 接受 none/xhigh。
- **阶梯基数不一致**：阶梯是二进制（1,048,576），厂商公布十进制，导致 **41 个 Go 条目中有 24 个永远无法提供自己的窗口**（1M 模型最高只能选 400K）。
- **两个同名但不同的路由被合并**：`glm-5.2` 与 `zai-org/GLM-5.2` 共享名称但 thinking 语义相反（实测 400 vs 200），一个设置会静默影响另一个。
- **标签冲突**：1,000,000 与 1,048,576 都显示为 `1M`。

### 验证

**498 个测试通过**（新增 40+ 个，覆盖上述每一个缺陷的回归）；`check:consistency` 干净。
...
| 设置 | 默认 | 含义 |
| --- | --- | --- |
| `groups.<key>.apiKeys` | `[]` | 额外凭据引用（`apiKey` 仍是主密钥，排在池首） |
| `groups.<key>.keyPolicy` | `sticky` | `sticky` 每会话固定一把；`round-robin` 每请求轮换 |

### 为什么默认粘性——这是缓存决策，不是负载决策

DSH 每一步都重发同一个不断增长的前缀，靠服务端 KV 缓存复用。轮询会让相邻步骤落到不同账号，**前缀缓存每步落空**，长任务的输入 token 开销被乘上数倍。所以默认粘性；轮询提供给「更在意分散额度消耗」的场景。

新会话按哈希分散到池中，避免单账号被抽干；无会话请求（列表/探测）无前缀可保，自动轮换。

### 故障切换

被上游以 `AUTH`/`RATE_LIMIT` 拒绝的密钥停用 60 秒后自动恢复。**只有凭据类失败才停用**：5xx、超时、请求格式错误不说明密钥有问题，停用它们反而会把健康账号踢出服务。会话固定的密钥被停用时改绑替补并保持，不来回抖动。

### 实现与边界

- 选择语义放在 **import-free** 的 `src/key-pool.ts`（与 `retry.ts` 同理：浏览器要读同一套策略词汇）。
- 池中每把引用都按族的凭据命名空间校验；重复引用被拒绝（否则冷却记账会出现「同一索引既停用又服务」的歧义）；池大小有上限。
- `keyPolicy` 在 resolver 里二次校验——程序化构造会绕过 Schemastery。
- 设置变更时池**原地重建**，存活会话的绑定关系不丢，避免白付一次冷缓存。
- 客户端新增密钥池文本域与策略下拉；文本域按「保存密钥池」按钮显式提交，不是每次击键都写。

全套 **351 项测试**通过（新增 `test/key-pool.spec.ts` 20 项、`test/key-pool-config.spec.ts` 11 项、client 用例 4 项）。
