	...
	*/
	select(request) {
		...
		for (let step = 0; step < this.keyCount; step += 1) {
			...
			this.lastPicked.set(request.sessionId ?? NON_SESSION, index);
			return index;
		}
		const fallback = this.soonestRecovering();
		this.lastPicked.set(request.sessionId ?? NON_SESSION, fallback);
		return fallback;
	}
...
function resolveAdapterOptions(config, family = PROTOCOM) {
	...
	for (const key of family.keys) {
		...
		for (const candidate of prospective) {
			...
			if (!family.credentialRef.test(candidate)) throw new Error(`${family.ns}: group "${key}" apiKey "${candidate}" must match ${String(family.credentialRef)}`);
			try {
...
var ProtocomAdapter = class extends LlmAdapter {
	...
	async capabilities(family) {
		...
		try {
			...
			if (!response.ok) value = scrapeCatalog(void 0, `the capability page answered HTTP ${response.status}`);
				...
				value = body.length > 8388608 ? scrapeCatalog(void 0, "the capability page was larger than the accepted bound") : scrapeCatalog(body);
			}
		} catch (error) {
	...
	*/
	...
	async modelInfoFor(group, provider, model) {
		...
		const reasoning = await this.modelCannotReason(group, upstreamId) ? void 0 : entry?.reasoning ?? await this.disclosedReasoning(group, upstreamId) ?? family.defaults[group.key]?.reasoning;
		return {
			provider,
			id: model,
...
var CommandCodeAccountService = class {
	...
	*/
	async read(group, signal) {
		...
		let usage;
		try {
			[credits, usage] = await Promise.all([COMMANDCODE.creditsPath === void 0 ? Promise.resolve({ error: "no credits endpoint configured" }) : readJson(root, COMMANDCODE.creditsPath, apiKey, signal), COMMANDCODE.usagePath === void 0 ? Promise.resolve({ error: "no usage endpoint configured" }) : readJson(root, COMMANDCODE.usagePath, apiKey, signal)]);
		} catch (error) {
