...
function wireMessage(message, images, replayReasoning) {
...
function serializeChatRequest(options, model, images, replayReasoning = false) {
	const messages = [];
...
async function* streamChatCompletions(connection, options, model, images, replayReasoning = false) {
	yield* translateChatCompletions(parseSse((await postSse(connection, "chat/completions", serializeChatRequest(options, model, images, replayReasoning), options.signal)).body));
}
//#endregion
...
var ProtocomAdapter = class extends LlmAdapter {
	...
	*/
	async protocolCall(connection, options, group, model) {
		...
		return group.protocol === "responses" ? streamResponses(connection, projected, model, images) : streamChatCompletions(connection, projected, model, images, group.replayReasoning);
	}
	/**
