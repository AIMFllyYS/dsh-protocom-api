...
*/
function acceptsImages(id, declared) {
...
*/
function catalogEntry(upstream, groupReasoning, declaredVision) {
...
function groupCatalog(key, listing, options = {}) {
	...
	const ranked = rows.filter((row) => servesChat(row.id) && options.hidden?.has(row.id) !== true).map((row, index) => ({
		index,
		row,
		rank: rankOf(row.id)
...
function resolveAdapterOptions(config) {
	...
	for (const [id, accepts] of Object.entries(config.visionModels ?? {})) {
		if (id.length === 0) throw new Error("protocom-api: visionModels keys must be non-empty model ids");
		if (typeof accepts !== "boolean") throw new Error(`protocom-api: visionModels["${id}"] must be a boolean`);
...
function serializeChatRequest(options, model, images, replayReasoning = false, assistantTextReplay = "keep") {
	const messages = [];
...
async function* streamChatCompletions(connection, options, model, images, replayReasoning = false, assistantTextReplay = "keep") {
	yield* translateChatCompletions(parseSse((await postSse(connection, "chat/completions", serializeChatRequest(options, model, images, replayReasoning, assistantTextReplay), options.signal)).body));
}
//#endregion
...
var ProtocomAdapter = class extends LlmAdapter {
	...
	*/
	async protocolCall(connection, options, group, model) {
		...
		return group.protocol === "responses" ? streamResponses(connection, projected, model, images) : streamChatCompletions(connection, projected, model, images, group.replayReasoning, group.assistantTextReplay);
	}
	/**
...
//#endregion
export { BalanceService, CONTEXT_LADDER, Config, DEFAULT_BASE_URL, DEFAULT_BASE_URL_ORIGIN, DEFAULT_RECOMMENDED, DEFAULT_STREAM_IDLE_TIMEOUT_MS, FALLBACK_CONTEXT_WINDOW, GROUP_DEFAULTS, GROUP_KEYS, PROTOCOM_CREDENTIAL_REF, ProtocomAdapter, REFUSED_CHAT_MODEL_IDS, REGISTRY, acceptsImages, apply, balanceFetchHandler, catalogEntry, contextChoicesFor, contextLabel, decodeVariantId, discoverModels, displayNameWithContext, encodeVariantId, endpointOrigin, fetchUpstreamModels, groupCatalog, groupOf, identityKey, inject, matchRegistry, modelIdentities, name, normalizeUsage, parseBalanceView, parseModelsListing, parseRateMultiplier, parseUsage, providerOf, resolveAdapterOptions, resolveBaseURL, servesChat, servesGroup, stripVariantId, variantLengths };
