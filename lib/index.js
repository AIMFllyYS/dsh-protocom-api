	...
	*/
	async listModels(provider) {
		...
		try {
			...
			for (const model of upstream) if (!known.has(model.id) && !retired.has(model.id)) rows.push(model);
		} catch {}
		const ranked = rows.filter((model) => !hiddenModels.has(model.id)).map((model, index) => ({
			index,
			model,
			rank: catalogEntry(model, GROUP_DEFAULTS[group.key].reasoning).rank
...
//#endregion
export { BalanceService, Config, DEFAULT_BASE_URL, FALLBACK_CONTEXT_WINDOW, GROUP_DEFAULTS, GROUP_KEYS, ProtocomAdapter, REGISTRY, RETIRED_MODELS, apply, balanceRouteHandler, catalogEntry, contextLabel, decodeVariantId, discoverModels, displayNameWithContext, encodeVariantId, fetchUpstreamModels, groupOf, inject, matchRegistry, modelIdentities, name, parseModelsListing, parseRateMultiplier, parseUsage, providerOf, resolveAdapterOptions, stripVariantId, variantLengths };
