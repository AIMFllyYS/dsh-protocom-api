...
import { contextLabel, contextStepsFor, groupCatalog, identityKey, matchRegistry, servesChat, } from "../model-registry.js";
...
function ModelRow({ model, group, family, hidden, recommended, contexts, vision, writable, busy, t, onWrite }) {
    ...
    // re-selectable. Deriving the chips from the stored set instead would make the
    // control vanish under the pointer that just used it.
    //
    // Evaluated by the same function the adapter uses to mint menu entries, so the
    // chips cannot describe a different set of entries from the ones that exist.
    ...
    return (_jsxs("div", { className: shown ? 'protocom-model-row' : 'protocom-model-row is-off', title: model.ids.join('\n'), children: [_jsxs("label", { className: "protocom-model-pick", children: [_jsx("input", { type: "checkbox", checked: shown, disabled: !writable || busy, "aria-label": model.displayName, onChange: toggleShown }), _jsx("span", { className: "protocom-model-dot" }), _jsx("span", { className: "protocom-model-name", children: model.displayName })] }), _jsx("span", { className: "protocom-model-spacer" }), _jsxs("span", { className: "protocom-model-summary", children: [_jsx("span", { className: "protocom-summary-ctx", children: chosen.map(length => contextLabel(length)).join(' · ') }), images ? _jsx("span", { className: "protocom-summary-tag", children: t('tagVision') }) : null, starred ? _jsx("span", { className: "protocom-summary-tag is-lead", children: t('tagLead') }) : null] }), _jsx("button", { type: "button", className: "protocom-model-more", "aria-expanded": openRow, "aria-label": t('rowSettings'), title: t('rowSettings'), onClick: () => { setOpenRow(!openRow); }, children: _jsx("span", { className: "protocom-caret", "aria-hidden": "true", children: openRow ? '▾' : '▸' }) }), openRow
                ? (_jsxs("div", { className: "protocom-model-detail", children: [_jsxs("span", { className: "protocom-detail-field", children: [_jsx("span", { className: "protocom-detail-label", children: t('contextTitle') }), _jsx("span", { className: "protocom-ctx", role: "group", "aria-label": t('contextTitle'), children: options.map((length) => {
                                    ...
                                    }) })] }), _jsxs("span", { className: "protocom-detail-field", children: [_jsx("span", { className: "protocom-detail-label", children: t('visionTitle') }), _jsx("button", { type: "button", className: images ? 'protocom-vision is-on' : 'protocom-vision', disabled: !writable || busy, "aria-pressed": images, onClick: toggleVision, children: images ? t('tagVision') : t('visionOff') })] }), _jsxs("span", { className: "protocom-detail-field", children: [_jsx("span", { className: "protocom-detail-label", children: t('leadTitle') }), _jsx("button", { type: "button", className: starred ? 'protocom-model-star is-on' : 'protocom-model-star', disabled: !writable || busy, "aria-pressed": starred, onClick: toggleStar, children: "\u2605" })] }), meta.length === 0 ? null : (_jsxs("span", { className: "protocom-detail-field", children: [_jsx("span", { className: "protocom-detail-label", children: t('capabilityTitle') }), _jsx("span", { className: "protocom-model-meta", children: meta })] }))] }))
                : null] }));
}
