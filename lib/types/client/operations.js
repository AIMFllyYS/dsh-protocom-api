...
export function createProtocomOperations(ctx, scope = { ns: PROTOCOM_ENTRY_ID, sectionKey: 'protocom' }) {
    ...
    return {
        ...
        writeSettings: async (ops, expectedRevision) => {
            ...
            if (response.ok) {
                // The reply already carries the committed Config, so hand the caller a
                // SECTION-shaped view exactly as describeSettings does. A caller that
                // applies this in place skips a second describe round trip -- and, more
                // importantly, skips the page-wide state replacement that made every
                // single toggle flash the whole panel.
                const value = response.value.value;
                const section = value !== null && typeof value === 'object' && !Array.isArray(value)
                    ? value[sectionKey]
                    : undefined;
                return { kind: 'written', view: { ...response.value, value: section ?? {} } };
            }
