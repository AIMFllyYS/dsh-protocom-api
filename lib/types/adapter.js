 ...
 */
import { IMAGE_OFFLOAD_REQUIRED_CODE, LlmAdapter, LlmError, offloadedImageText, projectOffloadedImages, ReasoningEffortId, requiredImageOffload, } from '@deepseek-ai/dsh-llm';
import { idleWatchdog, timeoutOf } from '@deepseek-ai/dsh-timeout';
import { requestImageDimensions } from '@deepseek-ai/dsh-attachment';
import { PROTOCOM } from "./family.js";
import { retryPolicyFor } from "./retry.js";
import { catalogFor, CATALOG_TTL_MS, MAX_CATALOG_BYTES, MAX_PLAN_BYTES, scrapeCatalog, tierFromPlanId, withinTier, } from "./commandcode-catalog.js";
import { acceptsImages, displayNameWithContext, contextStepsFor, protocolForEndpoints, FALLBACK_CONTEXT_WINDOW, groupCatalog, identityKey, matchRegistry, } from "./model-registry.js";
import { readBoundedBytes } from "./bounded-read.js";
import { decodeVariantId, encodeVariantId, stripVariantId } from "./context-variants.js";
 ...
 */
export function requestImageTargetFor(ref) {
    return {
...
export class ProtocomAdapter extends LlmAdapter {
    ...
    upstreamModels(group, signal) {
        ...
        const value = this.config.resolveApiKey(group)
            ...
            return rows.map((row) => {
                ...
                if (found === undefined)
                    return row;
                if (!withinTier(found.minPlan, tier))
                    return { ...row, outOfPlan: true };
                return {
                    ...row,
    ...
    contextLengthsFor(group, model) {
        ...
        if (group.contextLengths === undefined || group.contextLengths.length === 0)
            return undefined;
    ...
    async accountTier(family, group) {
        ...
        if (cached !== undefined && Date.now() - cached.at < PLAN_TTL_MS)
            return cached.value;
        let value;
        try {
            const { baseURL } = this.config.options();
    ...
    async resolveRequestImages(options, model) {
        ...
        if (refs.size === 0)
            return { images: undefined, messages: projected };
        ...
        if (attachments === undefined) {
            throw new LlmError(`${family.label} image input requires the durable attachment service.`, 'UNSUPPORTED_CONTENT');
        }
        ...
        if (offloadImages > 0) {
            throw new LlmError(`${family.label} request images exceed the route budget;`
                + ` ${offloadImages} more oldest occurrence(s) must be offloaded.`, IMAGE_OFFLOAD_REQUIRED_CODE, { offloadImages });
        }
