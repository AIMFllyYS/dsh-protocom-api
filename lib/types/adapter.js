...
import { catalogEntry, displayNameWithContext, FALLBACK_CONTEXT_WINDOW, matchRegistry, REGISTRY, RETIRED_MODELS, } from "./model-registry.js";
    ...
    async listModels(provider) {
        ...
        try {
            ...
            for (const model of upstream) {
                if (!known.has(model.id) && !retired.has(model.id))
                    rows.push(model);
            }
        }
        catch {
