 ...
 */
export const COMMANDCODE_TIERS = ['go', 'goat', 'pro', 'max'];
/**
...
export function tierFromPlanId(planId) {
    ...
    for (const tier of COMMANDCODE_TIERS) {
        ...
        if (new RegExp(`(^|[^a-z])${tier}([^a-z]|$)`).test(planId.toLowerCase()))
            return tier;
    }
    return undefined;
}
 ...
 */
export function withinTier(modelGate, accountTier) {
    const gate = tierRank(modelGate);
    const have = tierRank(accountTier);
    if (gate === undefined || have === undefined)
        return true;
    return gate <= have;
}
/**
