...
import { COMMANDCODE_BASE_URL, COMMANDCODE_RECOMMENDED } from "./commandcode.js";
...
export const SECTION_KEYS = ['protocom', 'opencodeGo', 'commandcode', 'fusion'];
...
export const CommandCodeSection = sectionSchema(COMMANDCODE_BASE_URL, COMMANDCODE_RECOMMENDED);
/**
    ...
    for (const key of family.keys) {
        ...
        for (const candidate of prospective) {
            ...
            if (!family.credentialRef.test(candidate)) {
                throw new Error(`${family.ns}: group "${key}" apiKey "${describeRejectedRef(candidate)}" must match ${String(family.credentialRef)}`);
            }
            try {
 ...
 */
export function describeRejectedRef(value) {
    // Above the longest real reference rather than at a round number: cutting
    // PROTOCOM_AGGREGATE_API_KEY would break the case the message exists for, so
    // the cut only ever lands on a value too long to be a reference at all.
    const LIMIT = 32;
