...
export const KEY_POLICIES = ['sticky', 'round-robin'];
/**
 ...
 */
export function hashSession(value) {
    let hash = 0;
...
export class KeyPool {
    ...
    select(request) {
        ...
        for (let step = 0; step < this.keyCount; step += 1) {
            ...
            if (this.isCoolingDown(index))
                continue;
            if (this.policy === 'sticky' && request.sessionId !== undefined)
                this.pin(request.sessionId, index);
    ...
    anchor(request) {
        if (request.sessionId !== undefined) {
            ...
            if (pinned !== undefined && pinned < this.keyCount)
                return pinned;
            if (pinned === undefined) {
