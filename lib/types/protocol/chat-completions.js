...
export function serializeChatRequest(options, model, images, replayReasoning = false) {
    const messages = [];
    if (options.system !== undefined)
        messages.push({ role: 'system', content: options.system });
    for (const message of options.messages)
        messages.push(wireMessage(message, images, replayReasoning));
    return {
        model,
        messages,
        stream: true,
...
export async function* streamChatCompletions(connection, options, model, images, replayReasoning = false) {
    const response = await postSse(connection, 'chat/completions', serializeChatRequest(options, model, images, replayReasoning), options.signal);
